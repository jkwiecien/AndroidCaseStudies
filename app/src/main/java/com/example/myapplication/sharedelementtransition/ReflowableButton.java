 ...
 */
public class ReflowableButton extends MaterialButton {
    ...
    public ReflowableButton(Context context) {
        this(context, null);
    }

    public ReflowableButton(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.textViewStyle);
    }

    public ReflowableButton(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        final TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.BaselineGridButton, defStyleAttr, 0);

        // first check TextAppearance for line height & font attributes
        if (a.hasValue(R.styleable.BaselineGridButton_android_textAppearance)) {
            int textAppearanceId =
                    a.getResourceId(R.styleable.BaselineGridButton_android_textAppearance,
                            android.R.style.TextAppearance);
            TypedArray ta = context.obtainStyledAttributes(
                    textAppearanceId, R.styleable.BaselineGridButton);
            parseTextAttrs(ta);
            ta.recycle();
        }
        ...
        parseTextAttrs(a);
        maxLinesByHeight = a.getBoolean(R.styleable.BaselineGridButton_maxLinesByHeight, false);
        a.recycle();
    ...
    private void parseTextAttrs(TypedArray a) {
        if (a.hasValue(R.styleable.BaselineGridButton_lineHeightMultiplierHint)) {
            lineHeightMultiplierHint =
                    a.getFloat(R.styleable.BaselineGridButton_lineHeightMultiplierHint, 1f);
        }
        if (a.hasValue(R.styleable.BaselineGridButton_lineHeightHint)) {
            lineHeightHint = a.getDimensionPixelSize(
                    R.styleable.BaselineGridButton_lineHeightHint, 0);
        }
        if (a.hasValue(R.styleable.BaselineGridButton_android_fontFamily)) {
            fontResId = a.getResourceId(R.styleable.BaselineGridButton_android_fontFamily, 0);
        }
    }
